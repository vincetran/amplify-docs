...
```java
// The callingActivity parameter should always be set if your app is
// expecting to use WebAuthn as not setting this option will lead to
// a sub-optimal user experience when authenticating via WebAuthn
...
The USER_AUTH sign in flow will support the following methods of first factor authentication: `AuthFactorType.WEB_AUTHN`, `AuthFactorType.EMAIL_OTP`, `AuthFactorType.SMS_OTP`, `AuthFactorType.PASSWORD`, and `AuthFactorType.PASSWORD_SRP`.
